...
def get_all_tickers():
    ...
    try:
        ...
        # 위키피디아 구조상 일반적으로 3번째 또는 4번째 테이블이 티커 목록입니다.
    ...
    # 러셀 2000 전 종목은 위키피디아에 상시 업데이트되지 않으므로, 주요 구성 종목 리스트를 가져오거나
    # 실패 시 기본 대형주/중형주 풀을 백업으로 활용합니다.
    try:
...
def screen_stocks(min_gain=0.10, max_gain=1.50):
    ...
    for i in range(0, len(tickers), chunk_size):
        ...
        try:
            ...
            time.sleep(1) # IP 차단 방지를 위한 미세 딜레이
        except Exception as e:
    ...
    if results:
        ...
        html_content += '<p><b>필터 조건:</b> 3년 전 ~ 1년 전 구간의 최고점을 최근 4주 내에 돌파하고, 현재 주가가 돌파선 대비 <b>+' + str(int(min_gain*100)) + '% ~ +' + str(int(max_gain*100)) + '%</b> 구간에 위치한 종목</p>'
...
if __name__ == "__main__":
    # 대량 조회의 경우 필터 범위를 적절히 조절하여 사용 가능합니다.
